...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        for lgpd in livingGhostPositionDistributions:
            maxProb = 0
            for pos, prob in lgpd.items():
                if prob > maxProb:
                    maxProb = prob
                    mostLikelyPos = pos
            mostLikely.append(mostLikelyPos)

        closestPos = ()
        minDist = float('inf')

        for pos in mostLikely:
        ...
        bestAction = None
        minPacmanDist = float('inf')

        for action in legal:
            nextPos = Actions.getSuccessor(pacmanPosition, action)
            
            if self.distancer.getDistance(nextPos, closestPos) < minPacmanDist:
                bestAction = action
                minPacmanDist = self.distancer.getDistance(nextPos, closestPos)
