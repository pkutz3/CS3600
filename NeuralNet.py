...
class Perceptron(object):
    ...
    def sigmoidActivation(self, inActs):
        ...
        acts = [1.0]
        acts += inActs
        return self.sigmoid(self.getWeightedSum(acts))
    ...
    def updateWeights(self, inActs, alpha, delta):
        ...
        for i, weight in enumerate(self.weights):
            nWeight = weight + (alpha * delta * acts[i])
            weights.append(nWeight)
            totalModification += abs(weight - nWeight)
        self.weights = weights
...
class NeuralNet(object):
    ...
    def feedForward(self, inActs):
        ...
        for layer in self.layers:
            p = [perceptron.sigmoidActivation(prev) for perceptron in layer]
            out.append(p)
            prev = p
        return out
        ...
        for example in examples:#for each example
            ...
            for layerNum in range(self.numHiddenLayers-1,-1,-1):
                ...
                for neuronNum in range(len(layer)):
                    ...
                    error = 0.0
                    for nextNum in range(len(nextLayer)):
                        error += deltas[0][nextNum] * nextLayer[nextNum].weights[neuronNum + 1]
                    delta = error * gPrime
                    """FILL IN - delta for this neuron
...
def buildNeuralNet(examples, alpha=0.1, weightChangeThreshold = 0.00008,hiddenLayerList = [1], maxItr = sys.maxsize, startNNet = None):
    ...
    for input, output in examplesTest:
        out = nnet.feedForward(input)[-1]
        equals = True
        for i in range(len(out)):
            if round(out[i]) != output[i]:
                equals = False
        if equals:
            testCorrect += 1
        else:
    ...
    testAccuracy = float(testCorrect) / float(testTotal)
