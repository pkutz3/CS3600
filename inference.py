...
class ParticleFilter(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        tmp = self.getBeliefDistribution()

        for pos in tmp.keys():
            tmp[pos] *= self.getObservationProb(observation, gameState.getPacmanPosition(), pos, self.getJailPosition())
...
class JointParticleFilter(ParticleFilter):
    ...
    def initializeUniformly(self, gameState):
        ...
        "*** YOUR CODE HERE ***"
        numP = self.numParticles
        lPos = list(itertools.product(self.legalPositions, repeat = self.numGhosts))
        random.shuffle(lPos)
    ...
    def observeUpdate(self, observation, gameState):
        ...
        if tmp.total() == 0.0:
            ...
            self.particles = [tmp.sample() for p in range(self.numParticles)]
