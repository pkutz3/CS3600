...
class CornersProblem(search.SearchProblem):
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.EAST, Directions.WEST, Directions.NORTH, Directions.SOUTH]:
            ...
            if not hitsWall:
                if nPos not in vc and nPos in self.corners:
                    vc += (nPos,)
                nextState = (nPos, vc)
                cost = 1
                successors.append((nextState, action, cost))
...
def cornersHeuristic(state, problem):
    ...
    while len(vc) < 4:
        ...
        for corner in corners:
            if not corner in vc:
                currDist = util.manhattanDistance(pos, corner)
                if heurDist > currDist:
                    heurDist = currDist
                    longCorner = corner
        heuristic += heurDist
        vc += (longCorner,)
...
def foodHeuristic(state, problem):
    ...
    maxFood = (foodList[0], foodList[0], 0)

    for food1 in foodList:
        for food2 in foodList:
            dist = util.manhattanDistance(food1, food2)
            if dist > maxFood[2]:
                maxFood = (food1, food2, dist)

    dist1 = util.manhattanDistance(position, maxFood[0])
    dist2 = util.manhattanDistance(position, maxFood[1])
    ...
    for food in foodList:
        xs = (far[0], near[0])
        ys = (far[1], near[1])
        if not (food[0] in xs or food[1] in ys):
            fncount += 1

    return finalDist + maxFood[2] + fncount*1.8
